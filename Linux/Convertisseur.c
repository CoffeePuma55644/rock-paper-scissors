...
void convevertion_hexadecimal(int nombre){ // Fonction qui convertit un nombre en hexadécimal
    int quotient, reste;
    char tab[100];
    int i = 0;
...
Début du programme avec les spécificitées suivantes

- Le programme demande un mot de passe à l'utilisateur
- Si le mot de passe est correct, l'utilisateur a accès au convertisseur
- Le convertisseur permet de convertir un nombre en binaire, en hexadécimal, des dollars en francs et des francs en dollars
- L'utilisateur peut quitter le convertisseur en tapant 5
- Si le mot de passe est incorrect, l'utilisateur n'a pas accès au convertisseur
...
int main(void){
    ...
    if (mot_de_passe == mot_de_passe_utilisateur)
    {
        ...
        case 2:
            ...
            int nombre2;
            scanf("%d", &nombre2);
            convevertion_hexadecimal(nombre2);
